    ...
    const fetchMember = async () => {
        ...
        try {
            ...
            if (paymentType === "Annual") {
                const { data } = await axios.get(`https://okhla-backend.onrender.com/Ohkla/getMemberAndPaymentSummaryById/${selectedId}/${selectedYear}`);
...
>
  ...
  />
      ...
      setChequeNo(val);
              ...
              />
              <InputField label="Bank" className="custom-input" value={bankName} onChange={(e) => {
      const val = e.target.value;
      setChequeNo(val);
    }} />
            </>
  ...
  <div style={{ flex: '0 0 48%' }}>
    ...
    <input
      ...
      onChange={(e) => {
        const val = e.target.value;
        setNewPayment(val); // ✅ this should match value
      }}
      required
